 ...
 *
 * Ownership is derived from the authenticated user's driver row. The caller never supplies a
 * driver id, so one driver cannot enumerate another driver's ledger by changing a URL or body.
 * The ledger itself is append-only in the schema; this module only reads and aggregates it.
 */
export async function getDriverEarningsForUser(
  ...
  const [entries, [aggregate]] = await Promise.all([
    db
      ...
      .orderBy(desc(driverEarnings.earnedOn), desc(driverEarnings.createdAt), desc(driverEarnings.id))
