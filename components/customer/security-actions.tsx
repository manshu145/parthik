    ...
    <div className="space-y-4">
      <div className="rounded-xl border p-4">
        ...
        <p className="text-muted-foreground mt-1 text-sm">
          End this browser session or revoke every active Parthik session.
        </p>
        <div className="mt-4 flex flex-wrap gap-3">
          <Button disabled={busy} onClick={() => signOut(false)}>
            Sign out
          </Button>
          <Button variant="outline" disabled={busy} onClick={() => signOut(true)}>
            Sign out everywhere
          </Button>
        </div>
        {error ? (
      ...
      <p className="text-muted-foreground text-xs">
        Demo credential authentication is active. Password reset and Firebase verification will be
        enabled during the final authentication cutover.
      </p>
    </div>
  );
}
