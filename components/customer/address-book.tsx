...
export function AddressBook() {
  ...
  async function mutate(id: string, method: 'PATCH' | 'DELETE') {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : 'Address update failed.');
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="grid gap-6 lg:grid-cols-2">
          ...
          items.map((item) => (
            <article key={item.id} className="rounded-xl border p-4 text-sm">
              <div className="flex items-start justify-between gap-3">
                ...
                <span
                  className={
                    item.isServiceable === false ? 'text-danger text-xs' : 'text-success text-xs'
                  }
                >
                  {item.isServiceable === false ? 'Not serviceable' : 'Serviceable'}
                </span>
              </div>
              <div className="mt-3 flex gap-2">
                {!item.isDefault ? (
                  <Button
                    ...
                    variant="secondary"
                    disabled={busy}
                    onClick={() => void mutate(item.id, 'PATCH')}
                  >
                    Make default
                  </Button>
                ) : null}
      ...
      <form onSubmit={create} className="space-y-3 rounded-xl border p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          {field('recipientPhone', 'Mobile number', true)}
          {field('pincode', 'PIN code', true)}
        </div>
        {field('line1', 'House, street and area', true)}
        ...
        {field('deliveryInstructions', 'Delivery instructions')}
        <label className="flex items-center gap-2 text-sm">
          <input
            type="checkbox"
            checked={form.isDefault}
            onChange={(event) =>
              setForm((value) => ({ ...value, isDefault: event.target.checked }))
            }
          />
          Make default
        </label>
