...
export function CategoryEditor({ rows }: { rows: CategoryRow[] }) {
    ...
    <div className="grid gap-4 lg:grid-cols-[360px_1fr]">
      <form onSubmit={save} className="space-y-3 rounded-xl border p-4">
        ...
        >
          ...
          {rows
            .filter((row) => !row.parentId && row.id !== editingId)
            .map((row) => (
          ...
          <tbody className="divide-y">
            {rows.map((row) => (
              <tr key={row.id}>
                ...
                <td className="px-4 py-3">
                  {rows.find((candidate) => candidate.id === row.parentId)?.name ?? 'Root'}
                </td>
...
export function BrandEditor({ rows }: { rows: BrandRow[] }) {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      const response = await fetch(
        '/api/v1/admin/config/brand' + (editingId ? '/' + editingId : ''),
        {
          method: editingId ? 'PATCH' : 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[360px_1fr]">
      <form onSubmit={save} className="space-y-3 rounded-xl border p-4">
        ...
        <Input
          value={form.logoKey}
          onChange={(e) => setForm({ ...form, logoKey: e.target.value })}
          placeholder="Logo asset key (optional)"
        />
        <label className="flex items-center gap-2 text-sm">
          <input
            type="checkbox"
...
export function ZoneEditor({ rows }: { rows: ZoneRow[] }) {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      const response = await fetch(
        '/api/v1/admin/config/zone' + (editingId ? '/' + editingId : ''),
        {
          method: editingId ? 'PATCH' : 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      <form onSubmit={save} className="space-y-3 rounded-xl border p-4">
        ...
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <Input
            ...
            onChange={(e) => setForm({ ...form, avgMinutes: e.target.value })}
            placeholder="Average delivery min"
          />
        </div>
        <textarea
          className="border-input bg-background min-h-20 w-full rounded-md border p-3 text-sm"
          ...
          placeholder="Serviceable pincodes, comma or space separated"
        />
        <label className="flex items-center gap-2 text-sm">
          <input
            type="checkbox"
          ...
          <tbody className="divide-y">
            {rows.map((row) => (
              <tr key={row.id}>
                ...
                <td className="px-4 py-3">{row.pincodes.length}</td>
                <td className="px-4 py-3">₹{(row.baseDeliveryFeePaise / 100).toFixed(2)}</td>
                <td className="px-4 py-3">₹{(row.minOrderPaise / 100).toFixed(2)}</td>
                <td className="px-4 py-3">{row.isActive ? 'ACTIVE' : 'INACTIVE'}</td>
                <td className="px-4 py-3">
                  <Button size="sm" variant="secondary" onClick={() => edit(row)}>
                    Edit
