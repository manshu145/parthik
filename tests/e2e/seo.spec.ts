...
test.describe('open graph and twitter', () => {
  ...
  test('generates the Hindi Open Graph image too', async ({ request }) => {
    // A single English card on every Hindi share is the one untranslated thing a
    // visitor would see before they even arrive.
    const response = await request.get('/hi/opengraph-image');
