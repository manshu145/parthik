...
export async function GET(request: Request) {
  ...
  try { await requireCurrentPermission('setting:manage_sensitive'); return apiSuccess({ settings: await listProviderSettings() }, { meta: { requestId } }); }
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const parsed = z.record(z.string(), z.string().max(10000)).safeParse(await request.json().catch(() => null));
    if (!parsed.success) return apiError(new Error('Invalid provider settings payload.'), { requestId });
