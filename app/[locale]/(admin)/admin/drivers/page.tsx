    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-4" data-testid="admin-drivers">
      <div>
        ...
        <p className="text-muted-foreground mt-1 text-sm">
          {locale === 'hi'
            ? 'Driver applications, availability और delivery performance.'
            : 'Driver applications, availability and delivery performance.'}
        </p>
      </div>
      ...
      {drivers.length === 0 ? (
        <Card>
          <CardContent className="p-4 text-sm">
            {locale === 'hi' ? 'अभी कोई driver नहीं है।' : 'No drivers yet.'}
          </CardContent>
        </Card>
      ) : (
              ...
              <thead className="bg-muted/50 text-muted-foreground text-xs">
                <tr>
                  ...
                  <th className="px-4 py-3 font-medium">
                    {locale === 'hi' ? 'उपलब्धता' : 'Availability'}
                  </th>
                  <th className="px-4 py-3 font-medium">
                    {locale === 'hi' ? 'डिलीवरी' : 'Deliveries'}
                  </th>
                  <th className="px-4 py-3 font-medium">
                    {locale === 'hi' ? 'बनाया गया' : 'Created'}
                  </th>
                </tr>
              </thead>
                ...
                {drivers.map((driver) => (
                  <tr key={driver.id} className="hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {driver.ratingAvg && (
                        <p className="text-muted-foreground mt-1 text-xs">★ {driver.ratingAvg}</p>
                      )}
                    </td>
                    <td className="text-muted-foreground px-4 py-3 text-xs">
                      {format.dateTime(driver.createdAt, {
                        dateStyle: 'medium',
                        timeStyle: 'short',
                      })}
                    </td>
