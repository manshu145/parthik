      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            {driver.driverCode} · {t('driverDetail')}
          </p>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <Card>
          <CardContent className="p-4 text-sm">
            ...
            <dl className="mt-3 grid gap-2">
              ...
              />
              <Row
                label={locale === 'hi' ? 'आपात संपर्क' : 'Emergency contact'}
                value={driver.emergencyContact ?? '—'}
              />
              <Row
                label={locale === 'hi' ? 'रेटिंग' : 'Rating'}
                value={driver.ratingAvg ? `${driver.ratingAvg} (${driver.ratingCount})` : '—'}
              />
            </dl>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4 text-sm">
            ...
            <dl className="mt-3 grid gap-2">
              <Row
                label={locale === 'hi' ? 'कुल डिलीवरी' : 'Total deliveries'}
                value={String(driver.totalDeliveries)}
              />
              <Row
                label={locale === 'hi' ? 'सफल डिलीवरी' : 'Successful deliveries'}
                value={String(driver.successfulDeliveries)}
              />
              <Row
                label={locale === 'hi' ? 'स्वीकृत' : 'Approved'}
                value={
                  driver.approvedAt
                    ? format.dateTime(driver.approvedAt, { dateStyle: 'medium' })
                    : '—'
                }
              />
              <Row
                label={locale === 'hi' ? 'बनाया गया' : 'Created'}
                value={format.dateTime(driver.createdAt, {
                  dateStyle: 'medium',
                  timeStyle: 'short',
                })}
              />
            ...
            {driver.rejectionReason && (
              <p className="text-danger mt-3 text-sm">{driver.rejectionReason}</p>
            )}
          </CardContent>
        </Card>
      </div>
