    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-4" data-testid="admin-vendors">
      <div>
        ...
        <p className="text-muted-foreground mt-1 text-sm">
          {locale === 'hi'
            ? 'Vendor applications, approval status और contact details.'
            : 'Vendor applications, approval status and contact details.'}
        </p>
      </div>
      ...
      {vendors.length === 0 ? (
        <Card>
          <CardContent className="p-4 text-sm">
            {locale === 'hi' ? 'अभी कोई vendor नहीं है।' : 'No vendors yet.'}
          </CardContent>
        </Card>
      ) : (
                ...
                {vendors.map((vendor) => (
                  <tr key={vendor.id} className="hover:bg-muted/30">
                    <td className="px-4 py-3">
                      ...
                      {vendor.legalName && (
                        <p className="text-muted-foreground mt-1 text-xs">{vendor.legalName}</p>
                      )}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="text-muted-foreground px-4 py-3 text-xs">
                      {format.dateTime(vendor.createdAt, {
                        dateStyle: 'medium',
                        timeStyle: 'short',
                      })}
                    </td>
