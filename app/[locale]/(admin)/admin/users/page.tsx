...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [rows, t] = await Promise.all([listAdminUserManagement(), getTranslations('adminNav')]);
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-4" data-testid="admin-users">
      <div>
        ...
        <p className="text-muted-foreground mt-1 text-sm">
          Promote verified accounts to admin roles, suspend access and revoke active sessions. New
          accounts are created only through verified authentication.
        </p>
      </div>
      <AdminUserManagement
        users={rows.map((row) => ({
