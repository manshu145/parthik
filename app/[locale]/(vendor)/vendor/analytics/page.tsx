...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-4" data-testid="vendor-analytics">
      <div>
        <h1 className="text-xl font-semibold">{t('analytics')}</h1>
        <p className="text-muted-foreground mt-1 text-sm">
          {locale === 'hi'
            ? 'Orders, delivered GMV, products और calculated vendor payout का live operational snapshot।'
            : 'Live operational snapshot of orders, delivered GMV, products and calculated vendor payout.'}
        </p>
      </div>
