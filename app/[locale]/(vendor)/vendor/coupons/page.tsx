      ...
      {rows.length === 0 ? (
        <Card>
          <CardContent className="p-4 text-sm">
            No coupons currently apply to this vendor.
          </CardContent>
        </Card>
      ) : (
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-muted/50">
              <tr>
                ...
                <th className="p-3">Minimum cart</th>
            ...
            <tbody className="divide-y">
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="p-3">{row.couponType}</td>
                  <td className="p-3">{row.discountValue}</td>
                  <td className="p-3">₹{(row.minCartPaise / 100).toFixed(2)}</td>
                  <td className="p-3">
                    {row.usedCount}
